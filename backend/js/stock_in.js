function addItem()
{
    ...
    if(pid=="" || qty=="" || pcode==undefined)
    {
        ...
        tr += "<td>" + qty + "</td>";
        tr += "<td>" + unit + "</td>";
        tr += "<td><a href='#' onclick='removeItem(this, event)' class='btn btn-danger btn-sm '>លុប</a>";
        tr +="&nbsp;<a href='#' data-toggle='modal' data-target='#editModal' onclick='editItem(this, event)' class='btn btn-success btn-sm '>កែប្រែ</a></td>";
        tr += "</tr>";
        let trs = $("#data tr");
        if(trs.length>0)
        {
