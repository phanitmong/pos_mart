        ...
        <table class="table table-sm table-hover table-bordered" id="tblData">
           ...
           <tbody>
                @foreach ($stock_in as $ins )
                    <tr>
                        <td>{{$ins->IN_ID}}</td>
                        <td>{{$ins->referent_id}}</td>
                        <td>{{$ins->user->name}}</td>
                        <td>
                            <a href="{{ url('stock-in/detail/'.$ins->id) }}" class="btn btn-sm btn-info">ពិនិត្យ​ <i class="fa fa-eye"></i></a>
                            <a href="" class="btn btn-sm btn-danger"> លុប​​​ <i class="fa fa-trash"></i></a>
                        </td>
                    </tr>
                @endforeach
           </tbody>
